...
def obter_nivel_por_paciente():
    ...
    if paciente_ocupante in fila_emergencia:
        nivel_ocupante = 3
    if paciente_ocupante in fila_prioridade:
        nivel_ocupante = 2
    if paciente_ocupante in fila_normal:
        nivel_ocupante = 1
...
def remarcar_consulta():
    limpar_tela()
    global nome_medico; global dia; global dia_novo; global horario_novo; global chave_nova
    global paciente_ocupante; global nivel_ocupante; global nivel_risco_atual
    ...
    try:
        ...
        if paciente in fila_emergencia:
            nivel_risco_atual = 3
        elif paciente in fila_prioridade:
            nivel_risco_atual = 2
        else:
            nivel_risco_atual = 1
        ...
        if chave_nova not in medicos[nome_medico]["agenda"]:
            ...
            print(f"Situação do paciente atual: {nome_risco()}")
            ...
            if nivel_risco_atual > nivel_ocupante:
                ...
                dia = dia_novo
                chave_alvo = chave_nova
                dia_alvo = dia_novo
                horario_alvo = horario_novo
                del medicos[nome_medico]["agenda"][chave_antiga]

                if buscar_vaga_mais_proxima():
                    dia_deslocado = dia_novo
                    horario_deslocado = horario_novo
                    chave_deslocada = chave_nova

                    # Coloca o paciente deslocado na vaga encontrada e o paciente
                    # com maior risco no horário que ele solicitou.
                    medicos[nome_medico]["agenda"][chave_deslocada] = paciente_ocupante
                    medicos[nome_medico]["agenda"][chave_alvo] = paciente
                    salvar_dados()
                    ...
                    print(f"Consulta: Dia {dia_alvo} às {horario_alvo}")
                    print("\nPaciente Deslocado:")
                    print(f"{paciente_ocupante}")
                    print(f"Novo horário: dia {dia_deslocado} às {horario_deslocado}")
                    print("\nTudo foi reorganizado com Sucesso!")
                else:
                    medicos[nome_medico]["agenda"][chave_antiga] = paciente
                ...
                # Retira a consulta antiga antes de procurar a nova vaga.
                del medicos[nome_medico]["agenda"][chave_antiga]
                dia = dia_novo
        ...
        print("Redirecionando você de volta ao início...")
        time.sleep(2.5)
        return
