...
import os
import time
arquivo = "agenda.json"; nome_completo = ""
idade = 0
cpf = ""; nome_medico = ""; 
dia = 0
horario = ""; chave = ""
        ...
        if isinstance(medicos_salvos, dict):
            for nome, dados_medico in medicos.items():
                if nome in medicos_salvos and isinstance(medicos_salvos[nome], dict):
                    ...
                    dados_medico["especialidade"] = dados_antigos.get(
                        "especialidade",
                        dados_medico["especialidade"]
                    )
                    dados_medico["dias"] = dados_antigos.get(
                        "dias",
                        list(range(1, 32))
                    )
                    dados_medico["horarios"] = dados_antigos.get(
                        "horarios",
                        dados_medico["horarios"]
                    )
                    dados_medico["agenda"] = dados_antigos.get(
                        "agenda",
                        {}
                    )
...
def obter_nivel_por_paciente():
    ...
    if paciente_ocupante in fila_emergencia:
        ...
        nivel_ocupante = 2
    elif paciente_ocupante in fila_normal:
        nivel_ocupante = 1
...
def adicionar_na_fila():
    ...
    remover_das_filas()
    if nivel_risco_atual == 3:
        if paciente_ocupante not in fila_emergencia:
            fila_emergencia.append(paciente_ocupante)
    elif nivel_risco_atual == 2:
        if paciente_ocupante not in fila_prioridade:
            fila_prioridade.append(paciente_ocupante)
    elif nivel_risco_atual == 1:
        if paciente_ocupante not in fila_normal:
            fila_normal.append(paciente_ocupante)
...
def remover_nome_das_filas(nome_paciente):
    while nome_paciente in fila_normal:
        fila_normal.remove(nome_paciente)
    while nome_paciente in fila_prioridade:
        fila_prioridade.remove(nome_paciente)
    while nome_paciente in fila_emergencia:
        fila_emergencia.remove(nome_paciente)

def adicionar_paciente_fila_por_risco(nome_paciente, risco):
    remover_nome_das_filas(nome_paciente)
    if risco == 3:
        fila_emergencia.append(nome_paciente)
    elif risco == 2:
        fila_prioridade.append(nome_paciente)
    else:
        fila_normal.append(nome_paciente)

def paciente_tem_outra_consulta(nome_paciente):
    for medico in medicos.values():
        for paciente_agendado in medico["agenda"].values():
            if paciente_agendado == nome_paciente:
                return True
    return False
...
def fila_atendimento():
    ...
    while True:
        ...
        if resposta_fila == 1:
            print(
                f"O atendimento da fila sem risco "
                f"é de {len(fila_normal)} pessoas"
            )
        elif resposta_fila == 2:
            print(
                f"O atendimento da fila prioritária "
                f"é de {len(fila_prioridade)} pessoas"
            )
        elif resposta_fila == 3:
            print(
                f"O atendimento da fila de emergência "
                f"é de {len(fila_emergencia)} pessoas"
            )
...
def cancelar_consulta():
    ...
    print("=====================")
    nome_medico = input(
        "Qual o médico ao qual você quer cancelar a consulta? "
        "(Bernado, João, Samara...)\n>  "
    ).strip()
    ...
    try:
        ...
        if chave not in medicos[nome_medico]["agenda"]:
            print(
                f"Não achei consulta dia {dia} às {horario} "
                f"com {nome_medico}"
            )
            time.sleep(3)
            return
        ...
        if paciente != nome_completo:
            ...
            return
        print(
            f"\nEncontrado: {paciente} com Dr.{nome_medico} "
            f"dia {dia} às {horario}"
        )
        confirmacao = input(
            "Tem certeza que quer cancelar a sua consulta?\n>  "
        ).strip().lower()
        ...
        if not paciente_tem_outra_consulta(paciente):
            remover_nome_das_filas(paciente)
        salvar_dados()
        print(
            f"Consulta de {paciente} cancelada! "
            f"Horário {dia} {horario} liberado."
        )
        input("Aperte Enter para sair\n>  ")
    except ValueError:
        print("Dia inválido!")
...
def remarcar_consulta():
    global nome_medico; global dia; global horario; global chave; global dia_novo; 
    global horario_novo; global chave_nova; global paciente_ocupante; global nivel_ocupante
    global nivel_risco_atual
    limpar_tela()
    print("\n=====================")
    ...
    print("=====================")
    print(
        f"Olá {nome_completo}! Qual o nome do médico que iria "
        "te atender? (Bernado, Samara, João...)"
    )
    ...
    try:
        ...
        if chave not in medicos[nome_medico]["agenda"]:
            print(
                f"Não achei consulta com {nome_medico} "
                f"dia {dia} às {horario}"
            )
            time.sleep(2)
            return
        ...
        print(f"Encontrei: {paciente} com {nome_medico}")
        time.sleep(1)
        limpar_tela()
        print("---Consulta NOVA---")
        ...
        if chave_nova not in medicos[nome_medico]["agenda"]:
            ...
            print(
                f"{paciente} foi de {dia} {horario} --> "
                f"{dia_novo} {horario_novo} "
                f"com o Dr.{nome_medico}"
            )
            input("Aperte Enter para sair")
            return
        ...
        if paciente_ocupante in fila_emergencia:
            ...
            nivel_ocupante = 1
        print(
            f"\nEsse horário já está ocupado por: "
            f"{paciente_ocupante}"
        )
        print(
            f"Situação do paciente atual: "
            f"{nome_risco_ocupante()}"
        )
        if risco_paciente > nivel_ocupante:
            print("\nA nova consulta possui prioridade maior!")
            print(
                f"{paciente_ocupante} será direcionado "
                "para o horário mais próximo disponível"
            )
            del medicos[nome_medico]["agenda"][chave]
            ...
            for dia_teste in range(dia, 32):
                ...
                for horario_teste in medicos[nome_medico]["horarios"]:
                    ...
                    if (
                        chave_teste not in medicos[nome_medico]["agenda"]
                        and chave_teste != chave_ocupada
                    ):
                        dia_novo = dia_teste
            ...
            if encontrou_vaga:
                ...
                medicos[nome_medico]["agenda"][chave_ocupada] = paciente
                adicionar_paciente_fila_por_risco(
                    paciente,
                    risco_paciente
                )
                adicionar_paciente_fila_por_risco(
                ...
                print(
                    f"Consulta: Dia {dia_novo} "
                    f"às {horario_novo}"
                )
                print("\nPaciente Deslocado:")
                print(paciente_ocupante)
                print(
                    f"Novo horário: dia {dia_novo} "
                    f"às {horario_novo}"
                )
                print("\nTudo foi reorganizado com sucesso!")
            else:
                medicos[nome_medico]["agenda"][chave] = paciente
                print(
                    "Não foi possível encontrar uma vaga "
                    "para reorganizar as consultas."
                )
        else:
            print(
                "\nEsse horário pertence a uma pessoa com "
                "prioridade igual ou maior!"
            )
            ...
            if buscar_vaga_mais_proxima():
                ...
                salvar_dados()
                print(
                    "Remarcado para o horário mais próximo disponível: "
                    f"dia {dia_novo} às {horario_novo}."
                )
            else:
                medicos[nome_medico]["agenda"][chave] = paciente
                print(
                    "Não foi possível encontrar uma nova vaga. "
                    "Sua consulta antiga foi mantida."
                )
        input("Aperte Enter para sair")
    except ValueError:
        print(
            "A sua resposta é inválida "
            "(digite o dia em número). Tente novamente..."
        )
        time.sleep(2.5)
...
def marcar_consulta():
    ...
    print("=====================")
    print(
        f"Olá {nome_completo}! Primeiramente precisamos saber "
        "qual a situação de risco do(a) senhor(a)"
    )
    print("\nOpção 1: Sem risco")
    ...
    try:
        ...
        print(f"\nMédicos disponíveis: {list(medicos.keys())}")
        ...
        print(
            ...
            f"{medicos[nome_medico]['horarios']}"
        )
        horario = input(
            "Escolha o horário (ex: 8:00):\n>  "
        ).strip()
        if horario not in medicos[nome_medico]["horarios"]:
            print("Horário inválido!")
            time.sleep(2)
        ...
        if chave not in medicos[nome_medico]["agenda"]:
            ...
            salvar_dados()
            print(
                f"Consulta marcada como {nome_risco()} "
                f"com {nome_medico} dia {dia} às {horario}"
            )
            input("Aperte Enter para sair:\n>  ")
            return
        ...
        if nivel_risco_atual > nivel_ocupante:
            ...
            if buscar_vaga_mais_proxima():
                if chave_nova == chave:
                    print(
                        "Não foi possível encontrar uma vaga diferente."
                    )
                else:
                    medicos[nome_medico]["agenda"][chave_nova] = (
                        nome_completo
                    ...
                    salvar_dados()
                    print(
                        f"Consulta marcada para o horário mais próximo "
                        f"disponível: dia {dia_novo} às {horario_novo}."
                    )
            else:
                print(
                    "Não foi possível encontrar uma vaga "
                    "para a nova consulta."
                )
...
def principal():
    ...
    while True:
        ...
        print("Opção 0: Sair")
        print(
            f"Olá o senhor(a) {nome_completo}! "
            "Precisa escolher uma Opção (0-5):"
        )
        try:
...
def logar():
    ...
    print(
        "Para fazer login precisamos do seu nome/segundo nome/"
        "senha(CPF)/idade."
    )
    ...
    for paciente in pacientes:
        ...
        if (
            ...
            print(
                "Redirecionando você para nosso "
            ...
            return
    print(
        "A sua resposta não corresponde aos dados reais, "
...
def cadastrar():
    ...
    for paciente in pacientes:
        if (
            isinstance(paciente, dict)
            and paciente.get("cpf") == cpf_novo
        ):
    ...
    if idade < 0 or idade > 130:
        print("Digite uma idade válida entre 0 e 130.")
        time.sleep(2.5)
        return
    ...
    if resposta_cadastro in ("sim", "s"):
        print(
            f"Sua senha é {cpf} guarde ela para logar "
            "quando precisar."
        )
        print(
            "Redirecionando para nosso sistema "
            "de Pacientes..."
        )
        pacientes.append({
            "nome": nome_completo,
